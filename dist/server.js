...
app.get("/digipet/rehome", (req, res) => {
    ...
    if (digipet) {
        ...
        res.json({
            message: "You have successfully rehomed your Digipet. To hatch a new one, try /digipet/hatch.",
        });
    }
    else {
        res.json({
...
app.get("/digipet/ignore", (req, res) => {
    if (model_1.getDigipet()) {
        ...
        res.json({
            message: "You ignored your digipet :( . Look how its stats dropped. You are vile",
            digipet: model_1.getDigipet(),
        });
    }
    else {
        res.json({
            message: "You don't have a digipet to ignore (luckily)! Try hatching one with /digipet/hatch",
        });
    }
});
